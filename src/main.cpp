...
void run_advanced_leds_test(LEDS &leds) { 
    // LED Assessment Demo
...
void run_simple_leds_test(LEDS &leds) { 
...
void bluetooth_uart_init() { 
    uart_init(BT_UART_ID, 115200); // set the baud rate to match the RN481
    gpio_set_function(BT_TX_PIN, GPIO_FUNC_UART);
    gpio_set_function(BT_RX_PIN, GPIO_FUNC_UART);
}

void transmit_accel_data(LIS3DH &accel) { 
    AccelData raw_data = accel.read_accel_data();
    float x_g = accel.accel_data_to_g(raw_data.x);
    float y_g = accel.accel_data_to_g(raw_data.y);
    float z_g = accel.accel_data_to_g(raw_data.z);
    printf("x: %.2fg, y: %.2fg, z: %.2fg\n", x_g, y_g, z_g);

    char buffer[64];
    int len = snprintf(buffer, sizeof(buffer), "x:%.2f,y:%.2f,z:%.2f\n", x_g, y_g, z_g);
    uart_write_blocking(BT_UART_ID, (uint8_t*)buffer, len);
    sleep_ms(100);
}
...
void run_advanced_mic_app(LEDS &leds, MICROPHONE &mic, arm_rfft_instance_q15 &fft_instance) { 
    ...
    // 7. Visually display the results on the LEDs (advanced version)
...
int main()
{
    ...
    bluetooth_uart_init(); // initialise bluetooth communcation functionality

    int mode = 0; // tracks the current demo mode (0 = LED Driver; 1 = spirit level; 2 = mic_app)
    ...
    for (;;) {
        ...
        switch(mode) { 
            case 0: 
                // Test the LED Driver
                run_simple_leds_test(leds); 
                break; 
            case 1: 
                // Test the spirit level program (lis3dh Driver) 
                run_spirit_level(leds, accel); 
                transmit_accel_data(accel); // transmit over bluetooth (BLE)
                break; 
            case 2: 
