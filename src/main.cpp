...
void run_simple_mic_app(LEDS &leds, MICROPHONE &mic, arm_rfft_instance_q15 &fft_instance) {
    ...
    for (int i = 0; i < 1024; i++) { 
        ...
        time_domain[i] = ((int32_t)time_domain[i] * (int32_t)hanning_window[i]) >> 15;  // need to cast both up to int32_t to avoid overflow as well as right shift to  
    ...
    for (;;) {
        ...
        switch(mode) { 
            ...
            case 2: 
                ...
                run_simple_mic_app(leds, mic, fft_instance); 
                break; 
        }
