...
void LIS3DH::init() { 
    ...
    accel_write_register(LIS3DH_CTRL_REG1, 0x27); // 10Hz, all axises enabled
    
    // Measurement range - from datasheet CTRL_REG4 (23h = 0x23)
    accel_write_register(LIS3DH_CTRL_REG4, 0x00); // leave as default (±2g apprpriate for spirit level application)
}
...
AccelData LIS3DH::read_accel_data() { 
    ...
    data.x = (int16_t)(raw_data[0] | (raw_data[1] << 8)) >> 6; // from lecture notes
    data.y = (int16_t)(raw_data[2] | (raw_data[3] << 8)) >> 6; // M = 6 for LIS3DH as that's the number of padding zeros it adds
    data.z = (int16_t)(raw_data[4] | (raw_data[5] << 8)) >> 6; 
    return data;  
} 
